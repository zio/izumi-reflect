...
abstract class LightTypeTag private[reflect] (
  ...
  def serialize(): LightTypeTag.Serialized = {
    ...
    LightTypeTag.Serialized(hashCodeRef, strRef, strDBs, LightTypeTag.CURRENT_BINARY_FORMAT_VERSION)
  }
...
object LightTypeTag {
  final val CURRENT_BINARY_FORMAT_VERSION = 30

  @deprecated("Use CURRENT_BINARY_FORMAT_VERSION instead", "3.1.0")
  final val currentBinaryFormatVersion: Int = CURRENT_BINARY_FORMAT_VERSION
  ...
  def parse[T](hashCode: Int, refString: String, basesString: String, version: Int): LightTypeTag = {
    ...
    if (version >= 0 && version < CURRENT_BINARY_FORMAT_VERSION) {
    ...
    } else if (version == CURRENT_BINARY_FORMAT_VERSION) {
      ...
      throw new LinkageError(s"""Couldn't parse a `LightTypeTag` version=$version generated by a newer version of `izumi-reflect`,
                                ...
                                |
                                |Supported `version` range in this version: 0..$CURRENT_BINARY_FORMAT_VERSION""".stripMargin)
    }
  }
