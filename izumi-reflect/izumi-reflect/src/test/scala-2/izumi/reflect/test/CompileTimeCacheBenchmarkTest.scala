...
/** Compile-time cache correctness test for Scala 2. */
class CompileTimeCacheBenchmarkTest extends AnyWordSpec with TagAssertions {
  ...
  "Compile-time cache" should {
    ...
    "correctly handle compound types (with)" in {
      trait A
      trait B
      trait C
      
      val tag1 = Tag[A with B]
      val tag2 = Tag[A with B]
      
      assertSameStrict(tag1.tag, tag2.tag)
      
      val tag3 = Tag[A with B with C]
      val tag4 = Tag[A with B with C]
