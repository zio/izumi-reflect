...
class LTTRenderablesTest extends TagAssertions {
  ...
  "LTT renderables" should {
    ...
    "render complex lambdas using long form when using scalaStyledRepr" in {
      ...
      assert(swapOptionT == s"izumi.reflect.test.OptionT[[A] ➾ A,_]")
      assert(useInner == s"[A] ➾ izumi.reflect.test.OptionT[[A1] ➾ scala.util.Either[+A,+A1],A]")
      assert(useInner2 == s"[A,B] ➾ izumi.reflect.test.OptionT[[A1] ➾ scala.util.Either[+A,+A1],B]")
